...
# ── Feishu image sending ────────────────────────────────────────────────────

import urllib.request
import json

_FEISHU_APP_ID = "cli_a937b1e5c4785bdd"
_FEISHU_APP_SECRET = "qKP2qWlwoSa6Y1TQAtobTf4amcXyhpT8"
_FEISHU_UNION_ID = "on_3d750acdda054404027643b9d21b3b87"   # 徐大爷


def _get_feishu_token() -> str:
    req = urllib.request.Request(
        "https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal",
        data=json.dumps({"app_id": _FEISHU_APP_ID, "app_secret": _FEISHU_APP_SECRET}).encode(),
...
def send_image_to_feishu(image_path: str, token: str = None) -> str:
    """
    Upload an image to Feishu and send it to the configured user (徐大爷).
...
def _ensure_cjk_fonts():
    ...
    print(f"📦 Downloading Noto Sans CJK font (~30MB, first-time only)...", file=sys.stderr)
    try:
        urllib.request.urlretrieve(font_url, user_font_otf)
        print(f"✅ Font downloaded to {user_font_otf}", file=sys.stderr)
...
def render_bar_chart(
    ...
    if send_to_feishu:
        print("📤 Sending chart to Feishu...", file=sys.stderr)
        send_image_to_feishu(out)
